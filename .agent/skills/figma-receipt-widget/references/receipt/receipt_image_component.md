...
Use this Markdown as a neutral handoff spec for building the gallery-saved receipt image state in any Flutter project.

## Context

- This is the auto-saved-to-gallery variant of the receipt.
- It reuses the same body content as the standard receipt, but the top treatment changes.
- The design source is Figma node `106:2895`.
- The background source is Figma node `106:2897`.
- The card is square-edged and has no outer radius.
...
- `wiwallet-logo.svg` -> header brand mark
- `receipt_background.svg` -> receipt watermark/background
- `qr_code_example.png` -> QR image example

The sender/bank logo can be supplied by the host project as an optional replaceable asset.
...
- `headerLogoAssetPath = '<project asset>/wiwallet-logo.svg'`
...
- Outer radius: none
- Shadow: none
- Divider: 1px hairline with no border radius — matches the square-edged card
- Background: full-bleed SVG watermark
- Header: compact row with `Payment` on the left and logo on the right
...
- Right side: `wiwallet-logo.svg`
- Render the logo as a single SVG asset at `86x20`
- Do not split the logo into separate icon and text widgets
- Keep the header row vertically centered
- Keep 16px separation between left and right sides
...
- QR image size: 78x78
- Do not render `Scan to Verify` below the QR image in this state.
...
- Keep the watermark clipped to the card bounds.
